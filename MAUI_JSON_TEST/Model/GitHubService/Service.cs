...
namespace MAUI_JSON_TEST.GitHubService
{
    public class Service
    {
        ...
        public async Task<List<Respond>> SearchRepositories(string query, string sort,
            IProgress<double>? progress = null)
        {
            var url = "https://api.github.com/search/repositories?q=" + query + "&" + "sort=" + sort + "&per_page=" +
                      10;
            ...
            while ((read = await stream.ReadAsync(buffer, 0, buffer.Length)) > 0)
            {
                ...
                if (contentLength > 0)
                {
                    double progressValue = (double)totalRead / contentLength;
